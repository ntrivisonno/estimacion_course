...
psim = np.zeros((Np,Nsim))
t = np.linspace(0, Ts*Nsim, Nsim)
psim[0,:] = 2*np.tanh((t-30))+ 3.0
#psim[0,:] = 2.0
psim[1,:] = 4.0
#psim[1,:] = 2*np.tanh((t-30))+ 2.0
psim = psim.T

for i in range(Nsim):
    ysim[i] = h(xsim[i])
    #xsim[i+1] = np.array(F_RK4(xsim[i], usim[i], psim)).ravel()
    xsim[i+1] = np.array(F_RK4(xsim[i], usim[i], psim[i])).ravel()
...
np.savetxt('coef.txt', psim) # p[:,0] = k, p[:,1] = c
